...
import takeshi.db.model.OBotPlayingOn;
...
import takeshi.templates.TemplateCache;
...
public class BotContainer {
	...
	public synchronized void restartShard(int shardId) throws InterruptedException, LoginException, RateLimitedException {
		...
		schedule(() -> {
			...
			reportError(String.format("Quick, shard `%02d` is on %s, where are the %s'? Restarting the shard, off we go %s!", shardId, Emojibet.FIRE,
					Emojibet.FIRE_TRUCK, Emojibet.ROCKET));
		}, 5L, TimeUnit.SECONDS);
	}
	...
	public void reportStatus(int shardId, JDA.Status oldStatus, JDA.Status status) {
		...
		if (channel.getJDA().getStatus() == JDA.Status.CONNECTED) {
			...
			shard.queue.add(channel.sendMessage(String.format(Emojibet.SHARD_ICON + " `%0" + length + "d/%0" + length + "d` | ~~%s~~ -> %s", shardId,
					shards.length, oldStatus.toString(), status.toString())));
		}
	}
	...
	public void sendStatsToDiscordlistNet() {
		...
		Unirest.post("https://bots.discordlist.net/api.php").field("token", BotConfig.BOT_STATS_DISCORDLIST_NET_TOKEN).field("servers", totGuilds)
				.asStringAsync();
	}
	...
	public int getRecommendedShards() {
		try {
			HttpResponse<JsonNode> request = Unirest.get("https://discordapp.com/api/gateway/bot").header("Authorization", "Bot " + BotConfig.BOT_TOKEN)
	...
	/**
	 * After the bot is ready to go; reconnect to the voicechannels and start
