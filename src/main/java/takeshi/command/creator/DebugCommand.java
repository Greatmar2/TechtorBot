...
public class DebugCommand extends AbstractCommand {
	...
	public String[] getUsage() {
		return new String[] { "fixusernames, fixrelations ", };
	}
		...
		for (DiscordBot shard : shards) {
			for (User user : shard.getJda().getUsers()) {
				...
				if (usersCompleted % updateInterval == 0L) {
					try {
						message.editMessage(String.format(messageFormat, usersCompleted, totalUsers, (double) usersCompleted / (double) totalUsers * 100D))
								.complete(false);
	...
	private void fixMemberships(DiscordBot bot, MessageChannel channel) {
		...
		String messageFormat = Emojibet.INFORMATION + " Synchronized " + Emojibet.GUILD_JOIN + " %0" + guildLength + "d / %0" + guildLength + "d | "
		...
		for (DiscordBot shard : shards) {
			for (Guild guild : shard.getJda().getGuilds()) {
				for (Member member : guild.getMembers()) {
					...
					usersCompleted++;
					if (usersCompleted % updateInterval == 0L) {
						message.editMessage(String.format(messageFormat, guildsCompleted, totalGuilds, usersCompleted, totalUsers,
								(float) usersCompleted / (float) totalUsers * 100F)).complete();
					}
				}
				guildsCompleted++;
			}
		...
		message.editMessage(
				String.format(messageFormat, guildsCompleted, totalGuilds, usersCompleted, totalUsers, (float) usersCompleted / (float) totalUsers * 100F))
				.complete();
	}
}