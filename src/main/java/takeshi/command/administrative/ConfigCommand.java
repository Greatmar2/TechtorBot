...
/**
 * !config gets/sets the configuration of the bot
 */
public class ConfigCommand extends AbstractCommand implements ICommandReactionListener<PaginationInfo> {
	...
	private static MessageEmbed makeEmbedConfig(Guild guild, int activePage) {
		...
		b.setFooter("Page " + (activePage + 1) + " / " + maxPage + " | Press the buttons for other pages | These expire at ", null);
		...
		b.setColor(QuickEmbedBuilder.PERM_COL);
		b.setTimestamp(OffsetDateTime.now().plusMinutes(2));
		return b.build();
	}
		...
		if (args.length == 0 || tag != null || args[0].equals("page")) {
			...
			if (PermissionUtil.checkPermission((TextChannel) channel, ((TextChannel) channel).getGuild().getSelfMember(), Permission.MESSAGE_EMBED_LINKS)) {
				...
				bot.queue.add(channel.sendMessage(makeEmbedConfig(guild, activePage)),
						message -> bot.commandReactionHandler.addReactionListener(((TextChannel) channel).getGuild().getIdLong(), message,
								getReactionListener(author.getIdLong(), new PaginationInfo<Object>(1, maxPage, guild))));
				return "";
			}
