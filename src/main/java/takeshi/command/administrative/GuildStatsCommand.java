...
public class GuildStatsCommand extends AbstractCommand {
	...
	public String[] getUsage() {
		return new String[] { "stats         //stats!", "stats mini    //minified!", "stats users   //graph of when users joined!",
	...
	public String stringExecute(DiscordBot bot, String[] args, MessageChannel channel, User author, Message inputMessage) {
		...
		case "music":
			return DebugUtil.sendToHastebin(
					getPlayingOn(bot.getContainer(), userrank.isAtLeast(SimpleRank.BOT_ADMIN) || (args.length >= 2 && args[1].equalsIgnoreCase("guilds"))));
		case "activity":
		...
		case "users":
			...
			for (Member join : joins) {
				Date time = DateUtils.round(new Date(join.getTimeJoined().toInstant().toEpochMilli()), Calendar.DAY_OF_MONTH);
	...
	private String getPlayingOn(BotContainer container, boolean showGuildnames) {
		...
		return Templates.command.stats.playing_music_on.format(activeVoice) + "\n"
				+ Misc.makeAsciiTable(Arrays.asList("Discord Id", "Name", "users", "in voice"), body,
	...
	private String getTotalTable(DiscordBot bot, boolean minified) {
		...
		for (DiscordBot shard : bot.getContainer().getShards()) {
			...
			double requestPerSec = (requests) / (System.currentTimeMillis() / 1000D - bot.startupTimeStamp);
			...
			if (!minified) {
				body.add(Arrays.asList("" + shard.getShardId(), "" + numGuilds, "" + users, "" + channels, "" + voiceChannels,
						activeVoice == 0 ? "-" : "" + activeVoice, String.format("%.2f/s", requestPerSec)));
			} else {
		...
		if (bot.getContainer().getShards().length > 1) {
			...
			return Misc.makeAsciiTable(header, body, Arrays.asList("TOTAL", "" + totGuilds, "" + totUsers, "" + totChannels, "" + totVoice, "" + totActiveVoice,
					String.format("%.2f/s", totRequestPerSec)));
		}
