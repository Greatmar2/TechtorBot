...
/**
 * !current retrieves information about the currently playing track
 */
public class NowPlayingCommand extends AbstractCommand {
	...
	public String[] getUsage() {
		return new String[] { "current                 //info about the currently playing song",
				"current seek <time>     //go to specified timestamp of track (eg. 3m10s)",
				"current vote <1-10>     //Cast your vote to the song; 1=worst, 10=best", "current repeat          //repeats the currently playing song",
				"current update          //updates the now playing message every 10 seconds",
				"current updatetitle     //updates the topic of the music channel every 10 seconds", "current source          //Shows the source of the video",
				"current pm              //sends you a private message with the details", "", "current clear               //clears everything in the queue",
				"current clear admin         //check if clear is admin-only", "current clear admin toggle  //switch between admin-only and normal", };
	}
	...
	public String simpleExecute(DiscordBot bot, String[] args, MessageChannel channel, User author, Message inputMessage) {
		...
		if (args.length >= 1) {
			...
			case "pm":
				bot.out.sendPrivateMessage(author, "The track I'm playing now is: " + song.youtubeTitle + "\n"
		...
		ret.append(MusicUtil.getMediaplayerProgressbar(musicHandler.getCurrentSongStartTime(), musicHandler.getCurrentSongLength(), musicHandler.getVolume(),
				musicHandler.isPaused())).append("\n\n");
		...
		if (args.length == 1 && args[0].equals("update")) {
			final Future<?>[] f = { null };
			bot.queue.add(channel.sendMessage(ret.toString()), message -> {
				if (message == null) {
					return;
				}
				bot.scheduleRepeat(() -> {
					...
					bot.queue.add(message.editMessage((player.isInRepeatMode() ? "\uD83D\uDD02 " : "") + autoUpdateText + "\n"
							+ MusicUtil.getMediaplayerProgressbar(musicHandler.getCurrentSongStartTime(), musicHandler.getCurrentSongLength(),
									musicHandler.getVolume(), musicHandler.isPaused())
							+ "\n\n"));
				}, 10_000L, 10_000L);
			});
			return "";
			...
			if (player.isUpdateChannelTitle()) {
				...
				if (PermissionUtil.checkPermission(musicChannel, guild.getSelfMember(), Permission.MANAGE_CHANNEL)) {
					...
					bot.scheduleRepeat(() -> {
						...
						bot.queue.add(musicChannel.getManager()
								.setTopic((player.isInRepeatMode() ? "\uD83D\uDD02 " : "")
										+ MusicUtil.getMediaplayerProgressbar(musicHandler.getCurrentSongStartTime(), musicHandler.getCurrentSongLength(),
												musicHandler.getVolume(), musicHandler.isPaused())
