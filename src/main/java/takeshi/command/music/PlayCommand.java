...
/**
 * !play plays a youtube link yea.. play is probably not a good name at the
 * moment
 */
public class PlayCommand extends AbstractCommand implements ICommandCleanup {
	...
	public String[] getUsage() {
		return new String[] { "play <youtubelink>    //download and plays song", "play <part of title>  //shows search results",
	...
	public String simpleExecute(DiscordBot bot, String[] args, MessageChannel channel, User author, Message inputMessage) {
		...
		if (!isInVoiceWith(guild, author)) {
			...
			try {
				...
				if (!PermissionUtil.checkPermission(vc, guild.getSelfMember(), Permission.MANAGE_CHANNEL) && vc.getUserLimit() != 0
				...
				if (maxResultCount > 1 && PermissionUtil.checkPermission(txt, guild.getSelfMember(), Permission.MESSAGE_ADD_REACTION)) {
					...
					txt.sendMessage(ret).queue(msg -> {
						...
						for (Pair<String, String> reaction : reactions) {
							listener.registerReaction(reaction.getKey(), message -> {
								listener.disable();
