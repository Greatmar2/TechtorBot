...
import takeshi.db.controllers.CRank;
import takeshi.db.controllers.CUser;
import takeshi.db.controllers.CUserRank;
...
public class SecurityHandler {
	...
	public GuildCheckResult checkGuild(Guild guild) {
		...
		int bots = 0;
		int users = 0;
		if (TimeUtil.getCreationTime(guild.getOwner().getUser().getIdLong()).isBefore(OffsetDateTime.now().minusDays(BotConfig.GUILD_OWNER_MIN_ACCOUNT_AGE))) {
