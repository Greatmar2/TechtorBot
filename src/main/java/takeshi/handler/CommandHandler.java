...
import takeshi.db.controllers.CBlacklistCommand;
import takeshi.db.controllers.CBotEvent;
import takeshi.db.controllers.CCommandCooldown;
import takeshi.db.controllers.CCommandLog;
import takeshi.db.controllers.CGuild;
import takeshi.db.controllers.CUser;
import takeshi.db.model.OBlacklistCommand;
import takeshi.db.model.OBotEvent;
import takeshi.db.model.OCommandCooldown;
...
public class CommandHandler {
	...
	public static void process(DiscordBot bot, MessageChannel channel, User author, Message incomingMessage) {
		MessageBuilder outMsg = new MessageBuilder();
		boolean commandSuccess = true;
		boolean startedWithMention = false;
		int guildId = 0;
		String inputMessage = incomingMessage.getContentRaw();
		String commandUsed = "-";
		// String commandPrefix = DisUtil.getCommandPrefix(channel);
		// if (inputMessage.startsWith(commandPrefix)) {
		// inputMessage = inputMessage.substring(commandPrefix.length()).trim();
		// } else
		...
		if (commands.containsKey(input[0]) || commandsAlias.containsKey(input[0])) {
			...
			if (command.canBeDisabled() && isDisabled(guildId, channel.getIdLong(), command.getCommand())) {
				...
				if (GuildSettings.getBoolFor(channel, GSetting.SHOW_UNKNOWN_COMMANDS)) {
					outMsg.setContent(Templates.command.is_blacklisted.format(input[0]));
				}
			} else if (cooldown > 0) {
				outMsg.setContent(Templates.command.on_cooldown.format(TimeUtil.getRelativeTime((System.currentTimeMillis() / 1000L) + cooldown, false)));

			} else if (!hasRightVisibility(channel, command.getVisibility())) {
				if (channel instanceof PrivateChannel) {
					outMsg.setContent(Templates.command.not_for_private.formatGuild(channel));
				} else {
					outMsg.setContent(Templates.command.not_for_public.formatGuild(channel));
				}
			} else {
				MessageBuilder commandOutput;
				...
				if (BotConfig.BOT_COMMAND_LOGGING) {
					...
					if (channel instanceof TextChannel) {
						CCommandLog.saveLog(CUser.getCachedId(author.getIdLong(), EmojiUtils.shortCodify(author.getName())),
								CGuild.getCachedId(((TextChannel) channel).getGuild().getIdLong()), command.getCommand(),
			...
			commandUsed = input[0];
			outMsg.setContent(DisUtil.replaceTags(customCommands.get(input[0]), author, channel, args));
		...
		if (commandSuccess) {
			if (channel instanceof TextChannel) {
				...
				if (commandLogChannel != null && commandLogChannel.canTalk()) {
					bot.queue.add(commandLogChannel.sendMessage(String.format("%s **%s#%s** used %s `%s` in %s", Emojibet.USER, author.getName(),
							author.getDiscriminator(), Emojibet.KEYBOARD, commandUsed, tc.getAsMention())));
				}
	...
	/**
	 * checks if a command is on cooldown and returns the amount of seconds left
	 * before next usage
	 *
		...
		for (AbstractCommand command : commands.values()) {
			for (String alias : command.getAliases()) {
				if (!commandsAlias.containsKey(alias)) {
					...
					DiscordBot.LOGGER.warn(String.format("Duplicate alias found! The commands `%s` and `%s` use the alias `%s`", command.getCommand(),
							commandsAlias.get(alias).getCommand(), alias));
				}
			}
		}
	}
	...
	private static void loadCommands() {
		Reflections reflections = new Reflections("takeshi.command");
	...
	/**
	 * (re-)loads the guild-specific blacklisted commands Map<Integer, Map<String,
	 * HashSet<String>>>
	 */
