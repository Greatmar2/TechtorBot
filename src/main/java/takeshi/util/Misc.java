...
public class Misc {
	...
	/**
	 * returns a formatted string from a time in seconds
	 *
	...
	/**
	 * Joins an array of strings together to 1 starting at position x ending at
	 * position y
	 *
		...
		for (MessageReaction react : reactions) {
			...
			for (int i = 0; i < users.size(); i++) {
				...
				if (userId != thisUser.getIdLong()) {
					System.out.println("Removing " + message + " by " + userId);
					react.removeReaction().queueAfter(50l, TimeUnit.MILLISECONDS);
				}
			}
		}
	}
