...
public class ConnectFourGame extends AbstractGame<Connect4Turn> {
	...
	protected boolean isTheGameOver() {
		...
		for (int j = 0; j < ROWS - 3; j++) {
			for (int i = 0; i < COLS; i++) {
				if (this.board.getValue(i, j) == getActivePlayerIndex() && this.board.getValue(i, j + 1) == getActivePlayerIndex()
						&& this.board.getValue(i, j + 2) == getActivePlayerIndex() && this.board.getValue(i, j + 3) == getActivePlayerIndex()) {
		...
		for (int i = 0; i < COLS - 3; i++) {
			for (int j = 0; j < ROWS; j++) {
				if (this.board.getValue(i, j) == getActivePlayerIndex() && this.board.getValue(i + 1, j) == getActivePlayerIndex()
						&& this.board.getValue(i + 2, j) == getActivePlayerIndex() && this.board.getValue(i + 3, j) == getActivePlayerIndex()) {
					setWinner(getActivePlayerIndex());
					return true;
				}
			}
		...
		for (int i = 3; i < COLS; i++) {
			for (int j = 0; j < ROWS - 3; j++) {
				if (this.board.getValue(i, j) == getActivePlayerIndex() && this.board.getValue(i - 1, j + 1) == getActivePlayerIndex()
						&& this.board.getValue(i - 2, j + 2) == getActivePlayerIndex() && this.board.getValue(i - 3, j + 3) == getActivePlayerIndex()) {
					setWinner(getActivePlayerIndex());
					return true;
				}
			}
	...
	public String toString() {
		...
		if (getGameState().equals(GameState.IN_PROGRESS) || getGameState().equals(GameState.READY)) {
			...
			ret.append("To play use a **reaction** or type `").append(getLastPrefix()).append("game <columnnumber>`");
		}
